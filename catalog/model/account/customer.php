...
//		$output = sendingEmailTest($info);
		...
		if (in_array('account', (array)$this->config->get('config_mail_alert'))) {
			...
			$info = array();
			$info["body"] = $message;
			$info["subject"] = html_entity_decode($this->language->get('text_new_customer'), ENT_QUOTES, 'UTF-8');
			$info["sender"] = html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8');
			$info["from"] = $this->config->get('config_email');
	...
	public function verifyCurrentPassword($email,$password){
		print("hola");
		$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1' AND approved = '1'");
		print_r($customer_query->num_rows);
	...
	public function getTotalCustomersByIdentityNumber($identityNumber) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(identitynumber) = '" . $this->db->escape(utf8_strtolower($identityNumber)) . "'");
		return $query->row['total'];
	}

	public function getTotalCustomersByRUC($RUC) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(RUC) = '" . $this->db->escape(utf8_strtolower($RUC)) . "'");
		return $query->row['total'];
	}
